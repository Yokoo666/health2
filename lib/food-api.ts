...
class FoodAPI {
  ...
  async searchByBarcode(barcode: string): Promise<FoodProduct | null> {
    try {
      const response = await fetch(`${this.baseURL}/product/${barcode}?fields=code,product_name,product_name_es,generic_name,brands,categories,labels,quantity,serving_size,packaging,manufacturing_places,origins,image_url,image_front_url,image_nutrition_url,nutrition_grade,nova_group,ecoscore_grade,nutrient_levels,nutriments,ingredients_text,ingredients_text_es,allergens,traces,additives_tags,nutrition_data_per,serving_quantity`);
  ...
  async searchByText(query: string, page: number = 1): Promise<{ products: FoodProduct[]; count: number; page: number; pageSize: number }> {
    try {
      const pageSize = 20;
      const response = await fetch(`${this.baseURL}/search?search_terms=${encodeURIComponent(query)}&page=${page}&page_size=${pageSize}&fields=code,product_name,product_name_es,brands,categories,image_front_url,nutrition_grade,nova_group,nutriments&sort_by=popularity`);
